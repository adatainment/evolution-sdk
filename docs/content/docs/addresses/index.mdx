...
const paymentCred = new Core.KeyHash.KeyHash({ hash: new Uint8Array(28) });
...
```typescript twoslash
import { Address } from "@evolution-sdk/evolution/core/Address";
import { KeyHash } from "@evolution-sdk/evolution/core/KeyHash";
...
const stakingAddress = new Core.Address.Address({
  networkId: 1, // mainnet
  paymentCredential: paymentCred,
...
const paymentOnlyAddress = new Core.Address.Address({
  networkId: 1,
  paymentCredential: paymentCred
...
const paymentCred = new Core.KeyHash.KeyHash({ hash: new Uint8Array(28) });
const stakeCred = new Core.KeyHash.KeyHash({ hash: new Uint8Array(28) });

const address = new Core.Address.Address({
  networkId: 1,
  paymentCredential: paymentCred,
  stakingCredential: stakeCred // ← Enables staking
})
```
...
**For advanced use cases** requiring era-specific features, see the [Address Types guide →](/docs/addresses/address-types)

> **Note:** "Franken addresses" are not a separate era - they're just Base addresses constructed with independent payment and stake credentials. See [Franken Addresses](/docs/addresses/franken) for this advanced pattern.
...
**Advanced Patterns:**
- [Franken Addresses](/docs/addresses/franken) - Constructing addresses with split credential ownership

**Practical Guides:**
- [Conversion](/docs/addresses/conversion) - Transform between Bech32, hex, and bytes
- [Validation](/docs/addresses/validation) - Verify addresses before use

**Related Topics:**
- [Transactions](/docs/transactions) - Sending funds to addresses
- [Staking](/docs/staking) - Delegation and rewards
- [Wallets](/docs/wallets) - Key management and address derivation
