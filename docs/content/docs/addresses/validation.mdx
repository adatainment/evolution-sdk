...
function validateAddress(addressString: string) {
  try {
    const address = Core.Address.fromBech32(addressString);
...
// Usage
const isValid = validateNetwork("addr1qx2kd28nq8ac5prwg32hhvudlwggpgfp8utlyqxu6wqgz62f79qsdmm5dsknt9ecr5w468r9ey0fxwkdrwh08ly3tu9sy0f4qd", "mainnet"); // true
const isInvalid = validateNetwork("addr_test1qz...", "mainnet"); // false
```
...
function hasStakingCredential(addressString: string): boolean {
  try {
    const address = Core.Address.fromBech32(addressString);
    return Core.Address.hasStakingCredential(address);
  } catch {
    return false;
  }
}
...
function isEnterpriseAddress(addressString: string): boolean {
  try {
    const address = Core.Address.fromBech32(addressString);
    return Core.Address.isEnterprise(address);
  } catch {
    return false;
  }
}
...
// Usage
console.log(hasStakingCredential("addr1qx...")); // true for base addresses
console.log(isEnterpriseAddress("addr1vx...")); // true for enterprise addresses
```
...
function validateUserInput(input: string): string | null {
  ...
  try {
    Core.Address.fromBech32(trimmed);
    return trimmed;
  } catch (error) {
