...
async function completeWorkflow() {
  ...
  const totalBalance = finalUtxos.reduce(
    (sum, utxo) => sum + Assets.getAsset(utxo.assets, "lovelace"),
    0n
  );
...
const utxos = await providerClient.getUtxos("addr_test1...");
...
import { Cluster, Config, Genesis } from "@evolution-sdk/devnet";
import { Core, createClient, type SigningClient } from "@evolution-sdk/evolution";
import * as Assets from "@evolution-sdk/evolution/sdk/Assets";
...
describe("Transaction Tests", () => {
  ...
  let genesisUtxos: readonly UTxO.UTxO[];
  ...
  it("should handle multiple outputs", async () => {
    ...
    const signBuilder = await client
      .newTx()
      .payToAddress({
        address: "addr_test1_receiver_1",
        assets: Assets.fromLovelace(2_000_000n)
      })
      .payToAddress({
        address: "addr_test1_receiver_2",
...
const client2 = createClient({
  ...
  const signBuilder = await client1
    ...
    .build({ availableUtxos: [wallet1Utxo] });
  ...
  const receivedUtxo = wallet2Utxos.find(u => u.txHash === txHash);

  if (receivedUtxo) {
    console.log("Wallet 2 received:", Assets.getAsset(receivedUtxo.assets, "lovelace"), "lovelace");
  }
