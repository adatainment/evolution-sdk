...
console.log("NFT found at:", utxo.address);
console.log("Current owner UTxO:", utxo.txHash);
```
...
const delegation = await client.getDelegation("stake1uxy...");
...
async function getPortfolioBalance(addresses: string[]) {
  ...
  const totalLovelace = balances.reduce(
    (sum, b) => sum + b.lovelace,
    0n
  );
  ...
  for (const address of addresses) {
    ...
    if (utxos.length > 0) {
      const totalAmount = utxos.reduce((sum, utxo) => {
        const amount = utxo.assets[tokenUnit] || 0n;
        return sum + amount;
      }, 0n);
...
async function checkPoolDelegation(
  ...
  const results = await Promise.all(
    rewardAddresses.map(async (rewardAddress) => {
      ...
      return {
        rewardAddress,
        delegatedToTarget: delegation.poolId === targetPoolId,
        currentPool: delegation.poolId,
        isDelegated: delegation.poolId !== undefined,
...
async function safeGetUtxos(address: string) {
  try {
    const utxos = await client.getUtxos(address);
...
- [Submission](/docs/providers/submission) - Submit and monitor transactions
