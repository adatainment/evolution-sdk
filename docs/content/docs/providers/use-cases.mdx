...
export async function exploreAddress(
  ...
  const utxos = await client.getUtxos(
    Core.Address.fromBech32(addressBech32)
  );
...
export async function trackPortfolio(
  ...
  const wallets = await Promise.all(
    addressesBech32.map(async (addressBech32) => {
      ...
      const tokens = Array.from(tokenMap.entries()).map(
  ...
  const totalLovelace = wallets.reduce(
    (sum, wallet) => sum + wallet.lovelace,
    0n
  );
...
export async function handleSubmission(
  ...
  try {
    // Submit transaction
    const signedTx = Core.Transaction.fromCBORHex(signedTxCbor);
    ...
    return {
      success: true,
      txHash: Core.TransactionHash.toHex(txHash),
      confirmed
    };
  } catch (error: any) {
...
export async function trackDistribution(
  ...
  for (const addressBech32 of addresses) {
    const address = Core.Address.fromBech32(addressBech32);
  ...
  holders.forEach((holder) => {
    holder.percentage = totalSupply > 0n
      ? Number((holder.amount * 10000n) / totalSupply) / 100
      : 0;
  });
...
export async function trackNFTCollection(
  policyId: string,
  tokenNames: string[]
    ...
    tokenNames.map(async (tokenName) => {
      ...
      try {
        ...
        return {
          tokenId: tokenName,
          unit,
          owner: Core.Address.toBech32(utxo.address),
          txHash: Core.TransactionHash.toHex(utxo.transactionId),
...
export async function getCollectionStats(
  ...
  const uniqueOwners = new Set(
    owned.map((nft) => nft.owner).filter(Boolean)
  );
...
interface Delegator {
  ...
  poolId: Core.PoolKeyHash.PoolKeyHash | null;
  rewards: bigint;
}

export async function trackPoolDelegators(
  rewardAddresses: string[],
  targetPoolId: Core.PoolKeyHash.PoolKeyHash
): Promise<Delegator[]> {
  const delegators = await Promise.all(
    rewardAddresses.map(async (rewardAddress) => {
      try {
        const delegation = await client.getDelegation(Core.RewardAddress.RewardAddress.make(rewardAddress));
  ...
  return delegators.filter(
    (d) => d !== null && d.poolId !== null && Core.PoolKeyHash.toHex(d.poolId) === Core.PoolKeyHash.toHex(targetPoolId)
  ) as Delegator[];
}
...
export async function getPoolStats(
  ...
  const delegators = await trackPoolDelegators(
    rewardAddresses,
    targetPoolId
  );
  
  const totalRewards = delegators.reduce(
    (sum, d) => sum + d.rewards,
    0n
  );
...
export class ParameterMonitor {
  ...
  private detectChanges(
    prev: ParameterSnapshot,
    curr: ParameterSnapshot
  ) {
    if (prev.minFeeA !== curr.minFeeA) {
      console.log("Fee A changed:", {
        from: prev.minFeeA,
        to: curr.minFeeA
      });
    }
    ...
    if (prev.poolDeposit !== curr.poolDeposit) {
      console.log("Pool deposit changed:", {
        from: prev.poolDeposit,
        to: curr.poolDeposit
      });
    }
  }
...
interface ProviderClient {
  getUtxos: (address: Core.Address.Address) => Promise<any>;
  submitTx: (tx: Core.Transaction.Transaction) => Promise<Core.TransactionHash.TransactionHash>;
...
class ProviderWithFallback {
  ...
  constructor(
    configs: Array<{
      ...
      provider:
        ...
        | { type: "kupmios"; ogmiosUrl: string; kupoUrl: string }
        | { type: "koios"; baseUrl: string };
    }>
  ) {
    this.clients = configs.map(({ network, provider }) =>
      createClient({ network, provider })
    );
  }
  ...
  async getUtxos(address: Core.Address.Address) {
    return this.withFallback((client) => client.getUtxos(address));
  }
  
  async submitTx(tx: Core.Transaction.Transaction) {
...
const address = Core.Address.fromBech32("addr1qxy8sclc58rsck0pzsc0v4skmqjwuqsqpwfcvrdldl5sjvvhyltp7fk0fmtmrlnykgmhnzcns2msa2cmpvllzgqd2azqhpv8e4");
...
- [Querying](/docs/providers/querying) - Master query methods
