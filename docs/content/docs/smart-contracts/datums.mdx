...
```typescript twoslash
import { Address, Assets, Data, DatumHash, createClient } from "@evolution-sdk/evolution"
...
const tx = await client
  ...
  .payToAddress({
    address: Address.fromBech32("addr_test1wrm9x2dgvdau8vckj4duc89m638t8djmluqw5pdrFollw8qnmqsyu"),
    assets: Assets.fromLovelace(10_000_000n),
    datum: new DatumHash.DatumHash({ hash })
  })
  .build()
```
...
```typescript twoslash
import { Address, Data, TSchema, Bytes, createClient } from "@evolution-sdk/evolution"
...
for (const utxo of utxos) {
  if (utxo.datum) {
    // Inline datum is available directly
