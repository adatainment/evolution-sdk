...
```typescript twoslash
import { Address, Assets, Data, InlineDatum, createClient } from "@evolution-sdk/evolution"
...
```typescript twoslash
import { Address, Assets, Bytes, Data, TSchema, createClient } from "@evolution-sdk/evolution"
...
const tx = await client
  ...
  .payToAddress({
    ...
    datum: new InlineDatum.InlineDatum({ data: Data.constr(0n, []) }),
    script: validatorScript // Store script in UTxO for reference
  })
  .build()
...
- [Spending from Script](/docs/smart-contracts/spending) — Unlock the funds you locked
- [Datums](/docs/smart-contracts/datums) — Datum types and construction patterns
- [Reference Scripts](/docs/smart-contracts/reference-scripts) — Store scripts on-chain
