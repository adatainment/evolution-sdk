 ...
 */
export const groupByPolicy = (
  tokens: ReadonlyArray<TokenInfo>
): Map<string, ReadonlyArray<TokenInfo>> => {
...
export const calculateAdaSubdivision = (
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.logDebug(`[Unfrack.calculateAdaSubdivision] Above threshold, subdividing with ${percentages.length} percentages`)
  ...
  return Effect.gen(function* () {
    ...
    if (tokens.length === 0) {
      ...
      if (availableLovelace >= subdivideThreshold) {
        yield* Effect.logDebug(`[Unfrack] No tokens, but ADA (${availableLovelace}) >= threshold (${subdivideThreshold}), checking subdivision affordability`)
    ...
    if (remaining >= subdivideThreshold) {
      yield* Effect.logDebug(`[Unfrack] Remaining (${remaining}) >= threshold (${subdivideThreshold}), checking subdivision affordability`)
      ...
      if (remaining >= adaMinUTxO) {
        ...
        yield* Effect.logDebug(`[Unfrack] Insufficient ADA for standalone output (${remaining} < ${adaMinUTxO}), spreading across bundles`)
