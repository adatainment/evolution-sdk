...
describe("Data Module Tests", () => {
  ...
  describe("Plutus List", () => {
    ...
    it("should create a valid list with elements", () => {
      const list = [42n, Data.bytearray("deadbeef")]
      expect(list).toHaveLength(2)
      expect(Data.isList(list)).toBe(true)
    })

    it("should validate Plutus List with schema", () => {
      const list = [42n, Data.bytearray("cafe")]
      expect(Data.isList(list)).toBe(true)
    })
  })
  ...
  describe("Plutus Map", () => {
    ...
    it("should create a valid map with entries", () => {
      const map = Data.map([
        [Data.bytearray("cafe"), 42n],
        [99n, Data.bytearray("deadbeef")]
      ])
  ...
  describe("Constr", () => {
    ...
    it("should create a valid constructor with fields", () => {
      const constr = Data.constr(2n, [42n, Data.bytearray("deadbeef"), [99n]])
    ...
    it("should validate Constr with schema", () => {
      const constr = Data.constr(5n, [42n, Data.bytearray("cafe")])
      expect(Data.isConstr(constr)).toBe(true)
    })
...
describe("CBOR Encoding/Decoding", () => {
  const testCases = [
    ...
    },
    {
      ...
      value: Data.constr(1n, [42n, Data.bytearray("cafe")]),
    ...
    },
    {
      ...
      value: [1n, Data.bytearray("deadbeef"), []],
  ...
  describe("Standard Encoding/Decoding", () => {
    describe("Hex Encoding/Decoding", () => {
      it.each(testCases)("should round-trip $name via hex encoding", ({ expectedHex, value }) => {
    ...
    describe("Bytes Encoding/Decoding", () => {
      it.each(testCases)("should round-trip $name via bytes encoding", ({ value }) => {
        const encoded = Data.toCBORBytes(value)
  ...
  describe("matchConstr", () => {
    it("should match specific constructor indices", () => {
      ...
      const result = Data.matchConstr(constr, {
        ...
        _: (index, fields) => `Default case: index ${index} with ${fields.length} fields`
      })
...
describe("Complex Structures", () => {
  it("should handle nested structures", () => {
    ...
    const complex = Data.constr(0n, [
      [1n, 2n, Data.bytearray("cafe")],
      Data.map([
        [42n, [Data.bytearray("deadbeef")]],
        [Data.bytearray("deadbeef"), Data.constr(1n, [-999n])]
      ]),
      Data.constr(7n, [[], Data.map([])])
    ])

    const encoded = Data.toCBORHex(complex)
    const decoded = Data.fromCBORHex(encoded)
...
describe("CBOR Encoding Format Validation", () => {
  ...
  it("should handle empty collections correctly", () => {
    ...
    ;[emptyList, emptyMap, emptyConstr].forEach((data) => {
      const encoded = Data.toCBORBytes(data)
      const decoded = Data.fromCBORBytes(encoded)
...
describe("Large Data Structure Edge Cases", () => {
  it("should handle large lists", () => {
    ...
    const encoded = Data.toCBORBytes(largeList)
    const decoded = Data.fromCBORBytes(encoded)
  ...
  it("should handle large maps", () => {
    ...
    const entries = Array.from(
      { length: 100 },
      (_, i) => [BigInt(i), Data.bytearray(`${i.toString(16).padStart(4, "0")}`)] as [Data.Int, Data.ByteArray]
    )
    const largeMap = Data.map(entries)

    const encoded = Data.toCBORBytes(largeMap)
    const decoded = Data.fromCBORBytes(encoded)
  ...
  it("should handle constructors with many fields", () => {
    ...
    const encoded = Data.toCBORBytes(constr)
    const decoded = Data.fromCBORBytes(encoded)
...
describe("Error Handling and Edge Cases", () => {
  describe("Invalid CBOR Input", () => {
    ...
    it.each(invalidInputs)("should handle $name gracefully", ({ input, method }) => {
      expect(() => {
        if (method === "cborHex") {
          Data.fromCBORHex(input)
        }
      }).toThrow()
    })
  })
    ...
    it("should handle bytearray edge cases", () => {
      ...
      edgeCases.forEach(({ value }) => {
        expect(() => {
          ...
          const encoded = Data.toCBORBytes(bytes)
          const decoded = Data.fromCBORBytes(encoded)
...
describe("Type Safety and Validation", () => {
  ...
  describe("Schema Validation", () => {
    it("should validate all PlutusData types with their schemas", () => {
      ...
      expect(Data.isBytes(Data.bytearray("deadbeef"))).toBe(true)
...
describe("Edge Cases and Robustness", () => {
  describe("Deeply Nested Structures", () => {
    it("should handle moderate nesting levels", () => {
      ...
      expect(() => {
        const encoded = Data.toCBORBytes(nested)
        const decoded = Data.fromCBORBytes(encoded)
    ...
    it("should handle mixed nested structures", () => {
      ...
      const complex = Data.constr(0n, [
        [Data.map([[1n, Data.bytearray("cafe")]]), Data.constr(1n, [-999n])],
        Data.map([
          [
            Data.bytearray("deadbeef"), // Valid hex string
            [1n, 2n, 3n]
          ],
          [42n, Data.constr(2n, [])]
        ])
      ...
      const encoded = Data.toCBORBytes(complex)
      const decoded = Data.fromCBORBytes(encoded)
      expect(decoded).toEqual(complex)
    })
  })
  ...
  describe("Data Structure Consistency", () => {
    it("should maintain referential integrity", () => {
      ...
      const data1 = Data.constr(42n, [123n, Data.bytearray("cafe")])
      const data2 = Data.constr(42n, [123n, Data.bytearray("cafe")])

      expect(data1).toEqual(data2)

      const encoded1 = Data.toCBORBytes(data1)
      const encoded2 = Data.toCBORBytes(data2)
...
it("should handle canonical format roundtrip", () => {
  ...
  const decoded = Data.fromCBORHex(firstCanonical)

  const encoded = Data.toCBORHex(decoded, { mode: "canonical" })
  expect(encoded).toBe(firstCanonical)
  expect(Data.fromCBORHex(encoded)).toEqual(decoded)
  ...
  const secondDecoded = Data.fromCBORHex(secondCanonical)
  const secondEncoded = Data.toCBORHex(secondDecoded, { mode: "canonical" })
  expect(secondEncoded).toBe(secondCanonical)
  expect(Data.fromCBORHex(secondEncoded)).toEqual(secondDecoded)
})
...
it("should handle unsorted map and return sorted canonical format", () => {
  const unsorted = Data.constr(15n, [
    Data.map([
      ...
      [Data.bytearray("70f3d8f8803183c15033"), Data.bytearray("88472079a8ce")],
      [Data.bytearray("0f7f249c0a9f4f829d2e4f"), 13859100696864903302n],
      [Data.bytearray("07"), Data.bytearray("b649")]
    ]),
    6n,
    Data.map([[Data.bytearray("65b7f7"), 5432168958238743917n]])
  ])

  const encoded = Data.toCBORHex(unsorted, { mode: "canonical" })
